...
def insert_delimiters_every_n_words(text: str, n: int = 100, delimiter: str = "---") -> str:
    ...
    result.append(delimiter)
    return "\n".join(result)  # Join with newlines so delimiters are on their own lines
...
def find_top_chunks(chunks: list[str], question: str, top_k: int) -> list[str]:
    ...
    3) If LLM expansion fails, fall back to phrase-only search (double-quoted).
    4) If no phrases, fall back to simple keyword matching (last resort).
    """
    # 1) LLM expansion
    ...
    # 4) Last resort: Simple keyword matching on question words
    import string
    tokens = question.lower().translate(str.maketrans('', '', string.punctuation)).split()
    tokens = [t for t in tokens if len(t) > 2]  # Filter out short words like "is", "it"
    
    # Score ALL chunks (don't filter by score > 0, just sort by relevance)
    all_chunk_scores = []
    for idx, c in enumerate(chunks):
        c_lower = c.lower()
        score = sum(c_lower.count(token) for token in tokens)
        all_chunk_scores.append((score, idx, c))
    
    # Sort by score descending (highest relevance first) and return top_k
    all_chunk_scores.sort(key=lambda x: x[0], reverse=True)
    selected = [c for _, _, c in all_chunk_scores[: max(1, top_k)]]
    return selected
